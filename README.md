...
- Subsequent `--log` and `--output` files will increment _1, _2, _3 as you might expect.
...
- Restore operation for this incremental operation **is not automated** but it should be fairly easy to script the restore of each file in sequence since each file is just an array of documents.
